...
class VectorizedCritic(nn.Module):
    def __init__(self, state_dim,action_dim, num_critics=2, hidden_dim=(256,256,256,256,256), drop_rate = 0.0):
        ...
        self.l5 = VectorizedLinear(hidden_dim[3], hidden_dim[4], num_critics)
        self.qs = VectorizedLinear(hidden_dim[4], 1, num_critics)
    ...
    def forward(self, state, action):
        ...
        q_values = self.dropout(q_values)
        q_values = F.relu(self.l5(q_values))
        q_values = self.dropout(q_values)
...
class Agent(object):
    def __init__(self, state_dim, goal_dim, action_dim, max_action, hidden_dim=(256, 256), hidden_dim2=(256, 256, 256, 256, 256),
        ...
        self.critic = VectorizedCritic(state_dim + goal_dim, action_dim, ensemble_size, hidden_dim2, drop_rate).to(device)
    ...
    def train(self, replay_buffer, demos, normalizers=(0, 1, 0, 1), iterations=2):
        for it in range(iterations):
            ...
            with torch.no_grad():
                ...
                demos_noise = (torch.randn_like(demos_action) * self.policy_noise).clamp(-self.noise_clip,
                                                                                         self.noise_clip)
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                if self.method == "MCDropout":
                    ...
                    demos_Q = self.critic(demos_input, demos_policy_actions)[0] - 2*demos_Q_std
                    Q_dem = self.critic(demos_input, demos_action)[0] - 2 * Q_dem_std
                if self.method == "First":  # usual Q-Filter method
                    # take only one pair of Q values to compare from the ensembles
                    demos_Q = demos_Q_set[0]
                    Q_dem = Q_dem_set[0]
                if self.method == "Mean":
                    demos_Q = torch.mean(demos_Q_set, dim=0)
                    Q_dem = torch.mean(Q_dem_set, dim=0)
                if self.method == "Minimum":
                    demos_Q = demos_Q_set.min(0)[0]
                    Q_dem = Q_dem_set.min(0)[0]
                if self.method == "LCB":
                    demos_Q_std = torch.std(demos_Q_set, dim=0)
                    Q_dem_std = torch.std(Q_dem_set, dim=0)
