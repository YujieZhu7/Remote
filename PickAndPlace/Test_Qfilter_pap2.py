...
# normalisation is the same for interactions and demonstrations
# normals = (np.mean(demos_states), np.std(demos_states), np.mean(demos_goals), np.std(demos_goals))
# num_accept_demos = 0  # updated during training to monitor the utilization of the demonstration data.
# num_tot_demos = 0
while steps < max_steps + 1:
    ...
    episodes += 1
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/StdQfilter/RanNoise0.1/Qfilter_S{seed}_score", score_history)
